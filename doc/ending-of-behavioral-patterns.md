...
## 1.3 易混淆的模式
总的来说，大部分行为型模式在结构上呈现出了较大的差异性，毕竟他们出自于不同的目的，但其中也不乏一些特例。
...
# 二、行为型模式的主旋律
在关于结构型模式的讨论中，我们对于结构型模式和行为型模式的分类原则做了大致的分析，并且挨个分析了为什么属于结构型模型。在前面我们已经说了，行为型模式把关注点放在行为上，关注类或者对象之间该如何交互与通信，该如何划分职责。所有行为型模式都围绕着几个主旋律展开，这几个主旋律是封装变化、对象入参、对请求解耦和如何通信。
...
- 命令模式使用一个`Command`对象来定义一个发送者和一个接收者之间的绑定关系。这样使得一个发送者可以和多个不同的接收者在一起工作，这样就解除了接收者和发送者之间的耦合关系，使得发送者更易于复用；
- 观察者模式通过定义一个接口`Observer#update()`来通知目标对象发生的变化，从而将发送者`Observable`和接收者`Observer`解耦；
- 中介者模式提供一个中介对象`Mediator`，让请求发送者`Colleague1`和请求接收者`Colleague2`之间不再直接交互。这样发送者和接收者之间便没有了依赖关系，所有的请求都经过中介对象的路由分发给相应的接收者；
- 责任链模式则通过沿着一个接收者对象链路传递请求，间接的实现了请求发送者`Client`和请求接收者`ConcreteHandler`之间的解耦；
